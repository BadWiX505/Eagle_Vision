...
from storage.detections import ViolenceDetectionStore
...
# ---------------------------------------------------------------------------
# Violence detection evidence store
# ---------------------------------------------------------------------------
detection_store = ViolenceDetectionStore()
...
@app.route("/api/detections/violence")
def get_violence_detections():
    """Return all stored violence detections, newest-first."""
    return jsonify(detection_store.get_all())


@app.route("/api/detections/violence/<detection_id>/image")
def get_violence_detection_image(detection_id: str):
    """Serve the cropped snapshot JPEG for a stored violence detection."""
    # Build path from the module-level constant in the store module.
    import storage.detections as _sd
    image_path = os.path.join(_sd._SNAPSHOTS_DIR, f"{detection_id}.jpg")
        ...
        if metadata is not None:
            ...
            if violence_bboxes and detection_store.can_store(ACTIVE_CAMERA_ID):
                ...
                if frame is not None:
                    best = max(violence_bboxes, key=lambda b: b.confidence)
