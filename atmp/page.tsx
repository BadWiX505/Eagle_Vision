...
import { useViolenceDetections } from "@/lib/hooks/useViolenceDetections";
...
import ViolenceDetectionsPanel from "@/components/detections/ViolenceDetectionsPanel";

export default function DashboardPage() {
  useDetectionStream();
  useViolenceDetections();
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[#050c14]">
      ...
      {activeTab === "evidence" ? (
        /* ── Evidence tab ──────────────────────────────────────────────── */
        <div className="flex-1 min-h-0 overflow-hidden glass m-2 rounded-xl">
          <ViolenceDetectionsPanel />
        </div>
      ) : (
        /* ── Live tab ──────────────────────────────────────────────────── */
        <>
